  ...
  <v-container class="pa-0 system-bg" fluid style="height: 100vh">
    <v-row no-gutters class="h-100">
      <v-col cols="12" sm="4" class="d-none d-md-flex position-relative">
        <v-card height="100%" class="position-sticky bg-transparent w-100 pa-0" style="top: 0" flat rounded="0">
          <v-carousel hide-delimiters continuous cycle interval="10000" height="100%" :touch="false"
            :show-arrows="false">
            <v-carousel-item height="100%" v-for="img in shuffledImages" :key="img">
          ...
          <div class="w-100 h-100 overlay position-absolute d-flex flex-column">
            <div class="mt-auto pa-3">
              <v-img src="/logos/access-light-no-tag.png" lazy-src="/logos/access-light-no-tag.png" width="200"></v-img>
              ...
              <v-row class="justify-center">
                <v-col cols="12">
                  ...
                  <div class="d-flex justify-center ga-3 ga-md-10 text-caption">
                    <NuxtLink :to="{ name: redirect?.name, params: redirect.params }" class="text-black"
                      v-if="redirect">
                      <span>Return</span>
                    </NuxtLink>
                    <div class="" v-for="{ title, route } in links" :key="route">
