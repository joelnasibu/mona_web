<template>
  <section
    class="trails-page w-full py-12"
    :style="{
  ...
  >
    <div class="max-w-6xl mx-auto px-6 text-center">
      <h1 class="page-title text-gray-900 dark:text-white">Suggested Trails & Visits</h1>
      <p class="subtitle text-orange-500 dark:text-gray-300">
        Discover curated trails and visit recommendations to explore Nairobi’s vibrant markets and culture.
      </p>

      <div class="trails-grid mt-8">
        <div v-for="(trail, index) in trails" :key="index" class="trail-card">
          <component :is="trail.icon" class="trail-icon" />
          <h2 class="trail-title">{{ trail.title }}</h2>
          <p class="trail-description">{{ trail.description }}</p>

          <div class="trail-meta">
            <span class="duration">⏱ {{ trail.duration }}</span>
            <ul class="highlights">
              <li v-for="(highlight, i) in trail.highlights" :key="i" class="highlight-item">
                <Check size="18" color="#032F46" /> {{ highlight }}
              </li>
            </ul>
          </div>
        </div>
...
<script setup>
import { ref } from "vue"
import { MapPin, Footprints, ShoppingBag, Camera, Utensils, Trees, Check } from "lucide-vue-next"
